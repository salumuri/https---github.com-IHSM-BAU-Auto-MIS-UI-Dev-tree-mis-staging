    ...
    */
    filterUpdate(event) {
     ...
     const temp = this.tempData.filter(function (d) {
       return (d.emp_id.toLowerCase().indexOf(val) !== -1 || d.emp_name.toLowerCase().indexOf(val) !== -1 || d.designation.toLowerCase().indexOf(val) !== -1
...
resetService(){
  ...
  this.walotBatches = [];
   ...
   getServiceslist(typ){
    ...
    var wid = frmObj.wsname;var sid = frmObj.service;var state = frmObj.state;
      this.Apiservice.get('WorkAllotment/GetAssociateNames?wid='+wid+'&sid='+sid+'&ssid='+state).subscribe((data: any) => {
  ...
  udpateWorkstreamUsersStatus(workstream,service,bid){
    //alert('Wid -->'+ wid+ '  status-->'+ sts)
    this.spinner.show();
    var userId = localStorage.getItem('LoginId');

        this.Apiservice.get('Masters/UpdateWorkstreamUsersStatus?userId='+userId+'&workstream='+workstream+'&service='+service+'&state='+bid).subscribe((data: any) => {
  ...
  udpateTrueStatus(workstream,service,bid){
    //alert('Wid -->'+ wid+ '  status-->'+ sts)
    this.spinner.show();
    var userId = localStorage.getItem('LoginId');

        this.Apiservice.get('Masters/UpdateTrueStatus?userId='+userId+'&workstream='+workstream+'&service='+service+'&state='+bid).subscribe((data: any) => {
          console.log("Update status --->", data);
          this.globalConst.checkOriginAccess(data);
