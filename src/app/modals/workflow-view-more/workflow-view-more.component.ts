...
@Component({
  selector: 'app-workflow-view-more',
  templateUrl: './workflow-view-more.component.html',
  styleUrls: ['./workflow-view-more.component.scss']
})
export class WorkflowViewMoreComponent implements OnInit {
  @Input() fromParent;
  selectedRow : any;
  subAction : any = false;
  dataResponse : any = [];
  ...
  ngOnInit(): void {
    ...
    if(this.fromParent['rowdt']){
      ...
      this.Apiservice.get('Masters/GetCountOfWorkFlow_Map_info?wid='+this.selectedRow.wid+'&map='+this.selectedRow.map+
